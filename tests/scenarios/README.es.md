...
Este directorio contiene un arnés de pruebas en Python que construye entradas
sintéticas de FASTA, GFF, VCF (o TSV de iVar) y BAM a partir de definiciones
declarativas de escenarios, ejecuta `get_mnv` sobre cada una y comprueba el TSV
de salida frente a campos esperados predefinidos.

Es complementario a las pruebas unitarias/de integración de Rust en
...
completo sobre entradas realistas, incluyendo:

- Soporte de lecturas calculado a partir del CIGAR del BAM
- Agrupación de codones a través de uniones de exones (modelos de CDS multiexón)
...
- Un binario `get_mnv` compilado — por defecto la suite busca
  `target/debug/get_mnv`, luego `target/release/get_mnv`, luego
  `dist/get_mnv`, y finalmente `get_mnv` en el `PATH`. Anula esto con `GET_MNV=/path/to/get_mnv`.
...
# Usa samtools de un entorno conda
SAMTOOLS=/path/to/conda/envs/bio/bin/samtools python3 tests/scenarios/run.py
```

Los archivos intermedios (FASTA, GFF, VCF, BAM, la salida de `get_mnv`, el log)
de cada escenario se escriben bajo `tests/scenarios/work/<scenario_name>/` y se
sobrescriben en cada ejecución. El directorio está en `.gitignore`.

## Vista general visual (PDF)
...
multiexón, formatos de entrada, casos límite), precedidas por una portada, una
leyenda de glifos y un índice. Cada página de escenario muestra:

- las **lecturas mapeadas** como un pequeño pileup de alineamiento, marcando
  SNVs, inserciones, deleciones y saltos de intrón (CIGAR `N`), los genes/CDS
  solapantes y las variantes de entrada;
- una **pista de marco de lectura / aminoácidos** — tripletes de codón sombreados
  de forma alterna más el aminoácido de referencia por codón, con el AA alternativo
  en rojo cuando una variante hace que el codón sea no sinónimo (teniendo en cuenta
  hebra y fase);
- dos tablas: **"Sin get_mnv"** (las llamadas crudas del caller — posición, alelo,
  conjetura de marco basada en la longitud) frente a la verdadera **"salida de get_mnv"**,
  con un resumen de una línea **"Lo que añade get_mnv"** entre ambas.

El PDF generado y cualquier vista previa `plot_*.png` están en `.gitignore`;
regenéralos desde el script.

## Disposición del minigenoma

Todos los escenarios comparten una referencia sintética (`framework.REFERENCE_SEQ`)
compuesta por dos contigs diseñados para mantener la aritmética de codones explícita:
...
operaciones opcionales: sustitución SNV, inserción, deleción, salto de intrón) y
las filas TSV esperadas.

### SNP / MNV / SNP/MNV principales

| # | Name | What it validates |
|---|------|-------------------|
| 01 | `snp_simple` | SNV único `Ala10Thr` (geneA codón 10 GCT→ACT) con soporte de 20/20 lecturas |
| 02a | `snp_mnv_full_phasing` | Dos SNVs en el mismo codón, todas las lecturas portan ambos → `SNP/MNV`, `Ala10Ser`, lecturas MNV 20, lecturas solo-SNP 0 |
...
| 03 | `snp_mnv_mixed` | Fasaje parcial: 10/10/10 lecturas (ambos / solo SNV1 / solo SNV2) → frecuencias 0.3333 / 0.3333 / 0.3333 |

### Indels en marco y con frameshift
...
### Haplotipos complejos (indel + SNV/MNV en cis)

| # | Name | What it validates |
|---|------|-------------------|
| 06 | `indel_plus_snv_haplotype` | Indel + SNV a >3 bp de distancia: NO se fusionan en `complex_indel` (fuera de la ventana local de 3 bp) |
...
| 08 | `inframe_ins_inside_codon_with_mnv` | Inserción en marco dentro de un codón + MNV en el mismo codón: emite 5 filas, incluyendo 2 `complex_indel`, la fila MNV marcada como `Indel overlap` / `Unknown`, la inserción sola, y el `complex_indel` ins+SNV |
| 09 | `fs_del_with_snv_overlap` | Deleción con frameshift que solapa el codón + SNV: la fila del SNV obtiene `Indel overlap` / `Unknown`; la fila de la deleción sola tiene `Event Reads = 0` (ninguna lectura porta solo la del); el `complex_indel` lleva el frameshift `Ala10Cysfs` |

### Gen en hebra negativa (`geneB`)

| # | Name | What it validates |
|---|------|-------------------|
| 10 | `minus_snp_simple` | SNV único en `geneB` (pos 673 C>T) — codón 10 del mRNA `GCT`→`ACT` → `Ala10Thr` |
...
### CDS multiexón (`geneC`, con `--gff-features CDS`)

| # | Name | What it validates |
|---|------|-------------------|
| 13 | `multiexon_snp_exon2` | SNV en el exón 2 — confirma la resolución de la posición del codón consciente del transcrito |
| 14 | `multiexon_junction_snp` | SNV en el codón que abarca la unión (base 1 en el exón 1 pos 900) — requiere el modelo empalmado |
| 15 | `multiexon_junction_mnv` | MNV con una base en el exón 1 (pos 900) y otra en el exón 2 (pos 1002) — adyacentes en el mRNA empalmado pero a ~100 bp de distancia en el genoma. Validado mediante lecturas con N-CIGAR que abarcan el intrón |
...
### Casos límite de aminoácidos

| # | Name | What it validates |
|---|------|-------------------|
| 18 | `stop_gained_via_mnv` | MNV de tres SNVs en el codón 50 `GCT`→`TAA` → `Ala50Ter`, `Change Type = Stop gained` |
| 19 | `start_codon_altered` | SNV en el codón de inicio ATG en pos 2 → `Met1Thr`. NOTA: `get_mnv` no tiene un Change Type `Start lost` dedicado; la fila se reporta como `Non-synonymous` |
...
| # | Name | What it validates |
|---|------|-------------------|
| 21 | `intron_variant` | SNV en el intrón de geneC (pos 950) → anotado como `intergenic` bajo `--gff-features CDS` |
...
### Entrada multicontig y TSV de iVar

| # | Name | What it validates |
|---|------|-------------------|
| 26 | `multicontig` | Dos variantes en dos contigs diferentes (`chr_test` y `chr_test2`) producen dos filas correctamente anotadas |
| 27 | `ivar_tsv_snv` | Entrada TSV de iVar con un SNV simple — la misma fila anotada que el equivalente en VCF |
| 28 | `ivar_tsv_insertion` | La notación TSV de iVar `+GCT` se normaliza al alelo anclado al estilo VCF y se anota como `INS:30:+GCT` |
| 29 | `ivar_tsv_deletion` | La notación TSV de iVar `-G` se normaliza a `DEL:31:G` y produce la misma fila con frameshift |

### Refinamientos de indels (rama indels)

Estos ejercitan los refinamientos del manejo de indels añadidos en la rama
`indels` (commit 49d2f09): detección de stop para indels en marco y la
compuerta de propagación aguas abajo `--frameshift-min-freq`.

| # | Name | What it validates |
|---|------|-------------------|
| 30 | `stop_gained_inframe_ins` | Una inserción en marco de `TAA` tras la pos 30 forma un codón de stop prematuro → `Change Type = Stop gained`, `AA Changes = 10_11ins*` (en lugar del genérico `In-frame Indel`). Impulsado por `indel_stop_effect`, que compara el número de residuos de stop en la proteína ref frente a la alt |
...
| 33 | `fs_gate_high_freq_suppressed` | Entradas idénticas al escenario 32 pero ejecutado con `--frameshift-min-freq 0.5`. La deleción aguas arriba (`AF=0.20`) no pasa la compuerta, por lo que el frameshift **no** se propaga → el SNV aguas abajo es un simple `Synonymous` / `Ala13Ala` |

Los escenarios 32 y 33 son un par A/B: el mismo VCF (que ahora lleva `AF` en el
`INFO`) y el mismo BAM, diferenciándose solo en el flag `--frameshift-min-freq`.
Juntos muestran que la compuerta afecta solo a la propagación aguas abajo del
frameshift — la fila propia de la deleción aguas arriba permanece como
`Frameshift Indel` (`Ala11Leufs`) en ambos.
...
- [`framework.py`](framework.py) define los bloques de construcción:
  ...
  - `VcfRecord(af=...)` emite un campo INFO `AF=` (omitido por defecto) de modo
    que los escenarios pueden impulsar lógica condicionada por frecuencia como
    `--frameshift-min-freq`
...
2. Decide dónde cae la variante en el minigenoma (consulta la aritmética de
   codones de arriba). Posiciones comunes:
...
3. Construye los grupos de lecturas con las operaciones necesarias para soportar
   la variante en el BAM. Los tipos de `Op` son:
   - `Op("snv", pos=P, seq="X")` — sustituye la base en la pos de ref `P` por `X`
   - `Op("ins", pos=P, seq="ABC")` — inserta `ABC` tras la pos de ref `P`
   - `Op("del", pos=P, length=N)` — borra `N` bases comenzando en `P`
   - `Op("skip", pos=P, length=N)` — emite un CIGAR `N` (salto de intrón), para
     lecturas empalmadas en escenarios multiexón
...
5. Opcionalmente establece `expected_row_count` para afirmar el número total
   exacto de filas producidas — útil cuando la ventana de haplotipo local emite
   más de una fila (consulta los escenarios 08 y 09).
6. Añade el escenario a `ALL_SCENARIOS` y ejecuta `python3 run.py`.

El driver mostrará `PASS` o `FAIL` para cada escenario y, en caso de fallo,
imprimirá las filas reales producidas para que puedas ajustar la expectativa.

## Comportamientos conocidos documentados por estas pruebas

- `geneB` (hebra negativa) — las columnas `Reference Codon` y `MNV Codon`
  muestran la secuencia **genómica hacia adelante** (p. ej. `AGC`), no la
  secuencia del mRNA (`GCT`). El efecto en el AA sigue siendo correcto.
- Un CDS multiexón sin un atributo `Name=` en la feature de CDS reporta la
  ...
  estándar de NCBI/Ensembl incluyen `Name=`, por lo que esto solo afecta a los
  GFF hechos a mano.
...
- Los indels en marco que **crean o eliminan un codón de stop** se reportan como
  ...
  introducen un stop aguas abajo, por lo que la distinción no sería informativa).
- `--frameshift-min-freq F` controla la **propagación aguas abajo del frameshift**:
  ...
  propagar siempre, y los indels sin una frecuencia conocida siempre se propagan.
  La compuerta nunca cambia la clasificación propia del indel.
- Cuando no se proporciona `--gff-features` y el GFF contiene features `CDS`, se
  selecciona automáticamente el modelo de CDS consciente de fase/empalme
  (escenario 34); pasa `--gff-features gene` para conservar el comportamiento
  heredado de gen completo.
- Una variante aguas abajo de un stop prematuro introducido por un frameshift se
  reporta con Change Type `Downstream of premature stop` en lugar de llevar una
  anotación `(fs)` como si estuviera traducida (escenario 35). La propagación
  ordinaria del frameshift, en la que no se introduce un stop temprano, no cambia.
