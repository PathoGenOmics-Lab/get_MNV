...
| `Codon LD` | Read-level linkage disequilibrium (`D'`) between the codon's substitutions, over the molecules that observe every position. A co-occurrence ratio cannot separate a haplotype from an accident of frequency: two substitutions on 90% of molecules each are found together on 81% of them by arithmetic alone, and the ratio calls that 0.9. `D'` measures the excess over what the two frequencies predict, normalised by the most it could have been. `+1` = they travel together as far as their frequencies allow, so the MNV is one haplotype. `~0` = they co-occur exactly as chance predicts, so they merely share a codon. `-1` = they exclude each other: both present, never on one molecule, which in a haploid population is two competing lineages rather than one variant. With three substitutions the weakest pair decides, since the codon claims one molecule carries all of them. `-` when no molecule spans the codon, or when an allele is on every molecule or none and there is nothing to correlate. Only present with `--bam`. |
| `Codon LD p` | Two-tailed Fisher exact p-value for that table, so a `D'` of 1.0 from four molecules is not read as one from four hundred. `-` under the same conditions as the column above. Only present with `--bam`. |
