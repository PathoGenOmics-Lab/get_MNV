...
| `Haplotype LD p` | Valor p exacto de Fisher a dos colas para esa tabla, para que un `D'` de 1.0 con cuatro moléculas no se lea igual que uno con cuatrocientas. `-` en los mismos casos que la columna anterior. Solo aparece con `--bam`. |
