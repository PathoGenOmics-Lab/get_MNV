...
### Added
- **Interactive HTML report.** New `--report <FILE.html>` writes a single self-contained page for exploring the called variants: headline counts, variants per sample stacked by type, the Sequence Ontology consequence distribution, and a virtualised, sortable, searchable table with a per-variant detail panel (HGVS `g.`/`c.`/`p.`, Grantham, DBS class, NMD, codons, event components, read support). Filters drive the charts as well as the table, and the current selection can be exported as TSV. The page embeds its data and loads nothing external, so it opens offline by double-clicking; repeated fields are dictionary-encoded to keep cohort-sized reports in the low megabytes. With `--sample all` one report covers every sample. For cohorts processed one sample per run, `--report-from a.MNV.tsv b.MNV.tsv ...` aggregates existing TSV outputs into one report without re-running the pipeline.
