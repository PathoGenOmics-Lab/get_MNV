...
- **Synthetic ground-truth validation** (`tests/truth/`, new CI job). The differential suite compares against tools we did not write, which is the stronger check, but it cannot validate the one thing get_MNV exists for: no per-variant annotator combines two SNVs in a codon the way get_MNV does, so on exactly those calls the oracles disagree by design. This suite builds a genome with four genes (plus and minus strand, single-exon and spliced) and derives the expected answer by a different route from get_MNV's: mutate the genome, re-extract the CDS, translate, compare proteins, using a codon table written out in the suite. It covers every single substitution at every position of every codon, every pair in a codon, and the triple, plus 324 indels (deletions and insertions of 1 to 3 bases at anchors through every exon, including the edges where the initiator and terminator codons live, with the indel applied in CDS coordinate space): about 1900 cases, each checked on gene, amino-acid change, change type, both codons and the HGVS `c.` descriptor, in five seconds. It is a second implementation by the same hand, so it catches implementation mistakes rather than a shared misunderstanding, and the consequence labels follow the documented convention; the amino acid, the codons and the CDS coordinate are the independent parts. Pointed at the build before the codon-orientation fix it reports 240 disagreements, all on the minus-strand genes.
